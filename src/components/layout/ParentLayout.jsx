...
import { Home, Users, UserCircle2, LogOut, Globe, Menu, X } from 'lucide-react';
...
const NAV = [
  ...
  { to: '/parent/profile', icon: UserCircle2, label: 'My Profile' },
];
...
export default function ParentLayout() {
  ...
  const handleSignOut = async () => {
    await signOut(); toast.success('Signed out'); nav('/parent/login', { replace: true });
  };
      ...
      <aside className={cn('fixed inset-y-0 left-0 z-40 w-64 transform border-r border-rc-200 bg-white transition-transform md:static md:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
        <div className="flex h-full flex-col">
          <div className="border-b border-rc-200 px-5 py-4">
            ...
            <p className="mt-2 text-xs font-semibold uppercase tracking-[0.18em] text-rc-700">Parent Portal</p>
          </div>
          ...
          <div className="border-t border-rc-200 p-4">
            <div className="mb-3 rounded-lg bg-rc-50 p-3">
              <p className="text-xs font-semibold text-rc-900">{parent?.display_name}</p>
              <p className="mt-0.5 text-xs text-rc-500">{parent?.relationship || 'Guardian'} · {parent?.parent_code}</p>
            </div>
