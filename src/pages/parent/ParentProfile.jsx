...
export default function ParentProfile() {
  const { parent, changeOwnPin } = useAuth();
  const [params] = useSearchParams();
  const forced = params.get('force') === '1' || parent?.force_pin_reset;
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-rc-600">Your details and PIN.</p>
      </header>

      <div className="card">
        <h2 className="font-display text-lg font-bold text-rc-900">Details</h2>
        <div className="mt-3 grid gap-3 text-sm md:grid-cols-2">
          <Row label="Parent code" value={<span className="font-mono">{parent?.parent_code}</span>}/>
          <Row label="Full name"   value={parent?.display_name}/>
          <Row label="Relationship" value={parent?.relationship}/>
          <Row label="Phone"       value={parent?.phone}/>
          <Row label="WhatsApp"    value={parent?.whatsapp_phone || '—'}/>
          <Row label="Email"       value={parent?.email || '—'}/>
        </div>
        <p className="mt-4 text-xs text-rc-500">To correct any of these, contact the school office.</p>
      </div>
